...
/** BOM 主档表（与 `server/index.js` 中 INV_BOM_MASTER_TABLE 约定一致） */
function invBomMasterFrom() {
...
export function registerPurchaseQuotationRoutes(app, deps) {
  ...
  app.post('/api/supply-chain/purchase-quotations', async (req, res) => {
    ...
    try {
      ...
      if (meta.headerColNames.has('uname') && actor.uname) {
      ...
      if (meta.headerColNames.has('utruename') && actor.utruename) {
      ...
      const lineSkip = new Set([fk.toLowerCase(), 'pass', 'del'])
  ...
  app.put('/api/supply-chain/purchase-quotations', async (req, res) => {
    ...
    try {
      ...
      if (meta.headerColNames.has('uname') && actor.uname) {
        setParts.push(`${bracketIdent('uname')} = @uname`)
        updReq.input('uname', sql.NVarChar(50), actor.uname)
      }
      if (meta.headerColNames.has('utruename') && actor.utruename) {
        setParts.push(`${bracketIdent('utruename')} = @utruename`)
