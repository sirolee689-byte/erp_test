...
import { registerStockMovementStatsRoutes } from './stockMovementStatsHandlers.js'
...
function getCurrentUserFromReq(req) {
  ...
  return {
    ...
    auditUserName:
      info.auditUserName != null && String(info.auditUserName).trim()
        ? String(info.auditUserName)
        : String(info.userName ?? info.userCode ?? '').trim() || '',
    auditTruename:
      info.auditTruename != null && String(info.auditTruename).trim()
        ? String(info.auditTruename)
        : '',
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (meta.legacyLayout) {
      ...
      const qTruenameLegacy = meta.qb('truename')
      const legacyTruenameSel = qTruenameLegacy
        ? `LTRIM(RTRIM(CONVERT(nvarchar(100), ISNULL(u.${qTruenameLegacy}, N'')))) AS AuditTruename,`
      ...
      const qTruenameErp = meta.qb('truename')
    ...
    tokenStore.set(token, {
      ...
      // 业务表 uname / utruename（与 CONTEXT 审计三字段一致；查库可再覆盖）
      auditUserName: String(userRow.UserName ?? userRow.UserCode ?? ''),
...
registerStockMovementStatsRoutes(app, { getPool })
...
async function startServer() {
  ...
  console.log(`StockIn-AssistReturn-Batch-v1.0.1 ${bootAt} BOM配件字段映射：Describe/d_info/remark→info，d_code→reference，tax=0.08`)
