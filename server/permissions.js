...
const ALL_ACTIONS = ['view', 'add', 'edit', 'delete', 'audit', 'review', 'unreview', 'price', 'export', 'print']
  ...
  if (data && typeof data === 'object') {
    ...
    for (const [k, v] of Object.entries(data)) {
      ...
      for (const a of v) {
        ...
        if (!allowed.has(low)) {
          return { ok: false, msg: `非法操作：${a}（允许 view/add/edit/delete/audit/review/unreview/price/export/print/all）` }
        }
        acts.push(low)
      }
