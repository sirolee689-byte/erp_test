...
async function fetchStockStatsCategoryOptions(pool, keyword = '') {
  const req = pool.request()
  let kwSql = ''
  if (keyword) {
    req.input('kw', sql.NVarChar(400), likePattern(keyword))
...
function buildReportTempTableSql() {
  return `
    DECLARE @materialCode nvarchar(200) = LTRIM(RTRIM(ISNULL(@materialCodeRaw, N'')));
    DECLARE @materialName nvarchar(500) = LTRIM(RTRIM(ISNULL(@materialNameRaw, N'')));
    DECLARE @materialNameEn nvarchar(500) = LTRIM(RTRIM(ISNULL(@materialNameEnRaw, N'')));
    ...
    WHERE
      ...
      AND (
        @materialCategory = N''
        OR r.[kcaa05] LIKE @materialCategoryLike ESCAPE N'\\'
        OR r.categoryName LIKE @materialCategoryLike ESCAPE N'\\'
      )
...
export function registerStockStatsRoutes(app, { getPool }) {
  ...
  app.get('/api/stock-stats/category-options', async (req, res) => {
    try {
      const pool = await getPool()
      const list = await fetchStockStatsCategoryOptions(pool, text(req.query?.keyword))
      res.json({ code: 200, msg: 'success', data: { list } })
    } catch (err) {
  ...
  app.get('/api/stock-stats/report', async (req, res) => {
    try {
      ...
      dbReq.input('materialCategoryRaw', sql.NVarChar(100), text(req.query?.materialCategory))
      ...
      const result = await dbReq.query(buildReportTempTableSql())
