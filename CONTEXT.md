...
- **订单外协**、**订单外发** 时关联单号使用 `GET /api/sales-order/pi-suggest` PI 联想（与物料单同源，候选项含销售订单交货日期）；**其他外协** 时关联单号可空或手填任意内容。
...
- 外协订单数据列四行展示：第一行外协商；第二行含税标记、总项数、总数量；第三行含税总价、不含税总价、税点总价（带「元」）；第四行额外费用。独立「外协商」列保留。
- 币别列读取 `wxaj07`，显示币别名称快照。
- 审核状态显示 `pass`，文案为未审核、已审核。
- 操作记录显示修改人、修改时间等审计信息。
- 主列表支持展开明细；展开后明细列包含：序号（按当前可见行从 1 编号，与库内排序字段 `seq` 解耦；保存时按页面顺序重写 `seq`）、PI 号、款号 `kcaa01`、中文名称 `kcaa02`、数量 `wxak03`、不含税单价 `wxak04`、含税单价 `wxak041`、不含税金额 `wxak05`、含税金额 `wxak051`、税点 `tax`、交货日期 `delivery_date`、参考单号 `reference`、备注 `wxak06`。查看弹窗明细与额外费用清单序号规则同上。
...
- **订单外发 / 其他外协** 点批量添加提示「当前外协类型暂不支持批量添加，后续版本开放」。
