...
- 列表提供 **显示未审核** 开关，用于查询未审核、未删除的外协订单。
- 列表提供 **回收站** 开关，用于查询已删除的外协订单。
- 结案状态作为额外筛选项，包含：全部、未结案、已结案。
- 列表支持按单字段搜索：外协订单号 `wxaj01`、外协订单日期 `wxaj02`、关联单号 `wxaj04`、备注 `remark`。
- 列表支持全字段模糊搜索，同时匹配：外协订单号 `wxaj01`、外协日期 `wxaj02`、关联单号 `wxaj04`、外协商编码 `wxaj05`、客户名 `kehu`、币别名称快照、备注 `remark`、打印注释 `notes`。
- 列表提供外协商独立筛选，从供应商下拉选择，支持按编码或名称模糊匹配。
- 列表提供外协类型独立筛选，包含：全部、订单外协、订单外发、其他外协。
- 列表提供排序选项：按交货日期排序、按外协日期排序、按供应商排序。
- 列表显示交货日期距今天数。
- 交货日期距今天数只做列表提示，不影响审核、结案、删除等流程；未到期显示 `还有 N 天`，当天显示 `今天到期`，已逾期显示 `逾期 N 天` 且用红色，无交货日期显示 `-`。

### 列表与展开明细（已定）

- 外协订单主列表显示列包含：操作按钮、选择列、结案状态、外协类型、外协订单号、订单日期、交货日期、外协订单数据汇总、关联单号、币别、供应商/外协商、备注、打印注释、审核状态、操作记录。
- 操作按钮包含：编辑、查看、删除、结案、反结案、打印。
- 结案状态显示 `closed`，文案为已结案、未结案。
- 外协类型显示 `wxaj03`，文案为订单外协、订单外发、其他外协。
- 外协订单数据汇总显示：客户名、含税标记、总项数、总数量、含税总价、不含税总价、税点总价、额外费用。
- 币别列读取 `wxaj07`，显示币别名称快照。
- 审核状态显示 `pass`，文案为未审核、已审核。
- 操作记录显示修改人、修改时间等审计信息。
- 主列表支持展开明细；展开后明细列包含：序号 `seq`、PI 号、款号 `kcaa01`、中文名称 `kcaa02`、数量 `wxak03`、不含税单价 `wxak04`、含税单价 `wxak041`、不含税金额 `wxak05`、含税金额 `wxak051`、税点 `tax`、交货日期 `delivery_date`、参考单号 `reference`、备注 `wxak06`。
...
- 外协订单第一版查看、编辑在当前系统页面的大弹窗或抽屉中完成，不做浏览器原生新页。
- 打印单独打开打印页。
