...
- 入库单回收站只处理已软删除的待审核单据。
- 入库单第一版不提供财务复核入口；新建单据默认 `sp_flag=0`，但遇到已有 `sp_flag=1` 的单据时只允许查看，禁止编辑、反审核和删除。
...
- 入库单权限动作沿用新系统模型：`view` 控制列表、详情和打印，`add` 控制新增，`edit` 控制待审核未复核单据编辑，`audit` 控制审核和反审核，`delete` 控制软删除和恢复，`price` 控制价格字段，`export` 预留给后续真实导出。
