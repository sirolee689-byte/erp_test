...
try {
  ...
  // 顺便验证 New_UB_ERP_User 是否存在、是否可查询
  const users = await pool.request().query('SELECT TOP (5) * FROM dbo.[New_UB_ERP_User]')
  console.log(`✅ New_UB_ERP_User 查询成功（前 5 行），行数：${users.recordset?.length ?? 0}`)
