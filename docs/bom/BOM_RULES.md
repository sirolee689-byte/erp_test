...
- BOM 主列表「一键运算」写入 `bom_cost` 时，CUT 自身不落成本表，但 CUT 的 `kcac04` 会像普通父件一样持续传递并放大所有下层；该按钮不复刻旧 ASP 固定层级的历史例外。
- 批量运算与详情页运算仍保持普通口径：CUT 自身不落成本表，且 CUT 的 `kcac04` 按 1 跳过；CUT 上方普通父件倍率仍继续传递到下层。
- 树形预览的原始平铺数据可以保持展示口径；最终成本用量以实际调用的写库口径为准。
...
- 材料备料表读取 `UB_ERP_Bom_pi_cost` 的已运算 PI 成本用量，不写回 BOM 或库存。
- 物料单备料数量为 `SUM(kcac06 * temp)`；`temp` 为空或无法转换时按 `1`。
- 该数量已经是 PI 运算结果，材料备料表不得再次乘销售订单数量。
- 只取 `del=0/isok=1/kcaa12=1` 的有效备料材料。
- 物料单分配件模式以 `pq` 作为产品编码、`top_kcaa02` 作为动态配件列名，按 `PI + pq + 材料编码 + top_kcaa02` 汇总 `SUM(kcac06 * temp)`。
- 同一材料属于多个 `top_kcaa02` 时分别进入对应配件列，不拆分或合并；合计列为所有配件列数量之和。
- 出库单分配件先按 `PI + 材料` 汇总实际已审核出库数量，再按各配件 BOM 需求占比分摊。
- 分摊保留 6 位小数，最后一个配件承接舍入尾差；无有效 BOM 需求的实际出库进入“未匹配配件”。
