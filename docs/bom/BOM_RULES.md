...
- 普通「一键运算」和批量运算写入 `bom_cost` 时，CUT 自身不落成本表，且 CUT 的 `kcac04` 按 1 跳过；CUT 上方普通父件倍率仍继续传递到下层。
- 仅超级管理员可用的「一键运算(旧)」写入同一张 `bom_cost`，但 CUT 的 `kcac04` 会像普通父件一样持续传递并放大所有下层；该按钮不复刻旧 ASP 固定层级的历史例外。
