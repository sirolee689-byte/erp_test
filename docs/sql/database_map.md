...
| 打卡消费补录 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal_log` + `UB_ERP_Hr_staff`；经手人读取 ERP 主库 `New_UB_ERP_User.truename` | 人员选择只读 `staff.id/code/new_code/name/card_number/new_card_number/meal_type`，可按 `code/new_code/name/card_number/new_card_number` 模糊查询，限制 `del=0、pass=1` 且有卡号。保存不要求先报餐；员工字段由服务端重查，`uid←staff.id`、`employee_id←staff.code`、`employee_name←staff.name`、`card_id/new_card_id←旧卡/新卡`、`dis_meal_type←staff.meal_type`。补录写 `meal_type=2/3、edible_time=补录日零点、dtime=补录日、bc_info=消费成功、bl=1、del=1`，整批共用 `blsystemcode`，`addtime` 为草稿打开时间，`bltime` 为保存时间，`bluser` 为当前账号 `truename`，`bl_info` 为备注，`ip` 由服务端请求取得。待审核行不计入正式刷卡。 |
