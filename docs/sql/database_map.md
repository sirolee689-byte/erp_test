...
| 业务功能 | 物理表 | 关键字段 / 说明 |
|----------|--------|-----------------|
| 员工档案列表/详情 | `UB_ERP_Hr_staff`（可用 `.env` `HR_STAFF_TABLE` 覆盖） | `GET /api/hr/staff`、`GET /api/hr/staff/:code`；列表按 `pass`、`del` 筛选；可选 `in_bm` 按部门名称精确筛（空=全部；兼容旧档 GUID 空/错）；**不依赖** `status`/`leave_date`/`remark` 列（`remark` 按列探测，缺列回空）。`del='0'` 在职，`del='1'` 离职；`pass` 审核位。前端表单**无备注录入**。 |
| 办理离职 / 恢复在职 | 同上 | `DELETE /api/hr/staff/:code` → `del='1'` 并写 `deltime=YYYY-MM-DD HH:mm:ss`（已审也可，不封账号）；`PUT /api/hr/staff/restore` → `del='0'`。饭堂历史报表用 `deltime` 判定“已离职”标记。旧 `PUT .../leave/:id`（status 离职）前端已停用。 |
| 员工报餐系统登录与改密 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Hr_staff` | 当前试用配置为 `UB_ERP_V2.0`，正式切换时才改为 `ERP_UB`。`POST /api/dining/login` 按 `new_code` 精确读取，要求 `del=0/pass=1`，兼容校验旧表 `password`。会话内部保留 `id/code/new_code/name/in_bm/in_bm_systemcode/meal_type/card_number/new_card_number` 快照；登录/会话仅返回本人展示用 `cardNumber`（新卡优先），绝不返回密码。`PUT /api/dining/password` 以 `id + 旧 password` 原子更新明文 `password`，成功后失效该员工全部报餐会话。 |
| 报餐与刷卡配置 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining` | 报餐接口读取启用配置 `bc`；刷卡终端读取 `closed`、午餐 `two1/two2`、晚餐 `three1/three2`。`closed=0` 未报餐拒绝，`closed=1` 允许补餐。本期不写该表。 |
| 统一餐兼容菜单 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_dishes` + `_list` | 新系统首次提交某日某餐时复用当天 `del=0/pass=1/enable=1` 主档；没有主档则自动 INSERT。明细按 `dtime + lx(2午餐/3晚餐) + info=新报餐系统统一餐兼容项` 识别，写 `pass=0/del=0/enable=1` 和“午餐（统一餐）/晚餐（统一餐）”。不修改、删除旧菜式。 |
| 员工报餐 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` | `GET /api/dining/meals` 按会话员工 `uid/dis_dtime/dis_lx` 读取 `del=0/pass=1`，把旧系统一餐多行合并为一个状态。新提交每餐只 INSERT 一条；取消按员工+日期+餐次把全部有效行 UPDATE 为 `del=1`。新刷卡终端也只按员工+日期+餐别读取有效行，不依赖具体菜式。 |
| 每天订餐情况表 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `UB_ERP_Hr_staff` | `GET /api/canteen/reports/daily-orders` 按单个 `dis_dtime` 读取 `del=0/pass=1` 且 `dis_lx=2/3`，以 `uid` 优先、员工编码或卡号回退识别员工，将旧系统同餐多菜合并为一天一人一行并返回午晚餐标记。员工编码、部门、姓名按报餐快照优先，空值再从员工档案补齐；当前 `staff.del=1` 且 `dis_dtime` 晚于 `staff.deltime` 日期时，名称追加“（已离职）”。总人数按员工去重，午晚餐人数分别去重。不读取 `dis_date/dis_meal_type` 或菜式字段，不新增索引。 |
| 订餐未刷卡明细 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `UB_ERP_Dining_meal_log` + `UB_ERP_Hr_staff` | `GET /api/canteen/reports/missed-swipes` 按 `startDate/endDate/department` 查询有效报餐，按员工+日期+餐别合并旧系统多菜记录；仅 `meal_log.del=0` 的同员工同日期同餐别流水算已刷卡，待审核补录 `bl=1/del=1` 不算。先按 `uid` 匹配，历史缺失时以员工编码、新旧卡号回退；资料快照优先、空值回退员工档案，岗位取 `staff.position`，卡号按新卡优先。当前 `staff.del=1` 且报餐日期晚于 `staff.deltime` 日期时，名称追加“（已离职）”。部门候选由 `GET /api/canteen/reports/missed-swipes/departments` 读取在职已审员工档案；接口均要求 `canteen/reports:view`，不新增索引。 |
| 消费汇总 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `UB_ERP_Dining_meal_log` + `UB_ERP_Hr_staff` | `GET /api/canteen/reports/consumption-summary` 按 `startDate/endDate/department` 以 `staff.del=0、pass=1` 的在职已审核员工为完整名单，零报餐、零消费员工也返回；部门按 `staff.in_bm` 精确筛选。报餐和 `meal_log.del=0` 正式消费均按员工+日期+餐别去重，优先 `uid=staff.id`，缺失时回退新旧员工编号及新旧卡号。`bl=1、del=0` 是已审核人工补餐，同时计正式消费和补餐；待审核补录不计。接口返回午晚报餐、消费、补餐、报餐有/无消费次数以及按每餐 5 元计算的午晚金额、无消费金额、可补贴金额和代扣餐费；不修改补录规则，不新增索引。 |
| 月报餐统计表 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `UB_ERP_Hr_staff` | `GET /api/canteen/reports/monthly-orders` 按 `YYYY-MM` 统计当月每个自然日的午餐/晚餐总数、员工餐和管理餐；仅 `del=0/pass=1/dis_lx in (2,3)`，同一员工+日期+餐别先合并以兼容旧系统多菜记录。餐类优先取报餐快照 `dis_meal_type`，空值回退 `staff.meal_type`，仍空按员工餐；员工档案关联不以 `del/pass` 过滤，故离职员工离职前的有效报餐保留在历史月报。接口补齐没有报餐的日期为 0，不读菜式或消费流水，不新增索引。 |
| 后台报餐人记录搜索与取消 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `_meal_log` + `UB_ERP_Hr_staff` | `GET /api/canteen/records/people` 查询当前月及前两个月，按 `uid+dis_dtime+dis_lx` 合并旧系统多菜记录，以 `ROW_NUMBER()` 服务端分页；员工编码优先 `staff.new_code`，卡号按 `new_card_number→card_number` 回退。`DELETE /api/canteen/records/people/:uid/:date/:mealType` 只允许删除权限者取消当天及未来且尚无 `meal_log del=0` 流水的报餐；与刷卡共用 `DiningSwipe` 锁，再将该餐所有 `del=0/pass=1` 行软删除。 |
| 打卡消费补录 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal_log` + `UB_ERP_Dining_meal` + `UB_ERP_Hr_staff`；经手人读取 ERP 主库 `New_UB_ERP_User.truename` | 人员选择只读 `staff.id/code/new_code/name/card_number/new_card_number/meal_type`，可按 `code/new_code/name/card_number/new_card_number` 模糊查询，限制 `del=0、pass=1` 且有卡号。批量添加「保存已选」前 `POST /api/canteen/records/supplements/staff-check` 按补录日+餐别只读复查：`meal_log.del=0` 正式刷卡或 `bl=1/del=1` 待审补录则跳过；同时看同日同餐 `meal.del=0/pass=1` 是否有报餐，用于文案「已报餐且已刷卡」或「已存在有效刷卡」。最终保存不要求先报餐；员工字段由服务端重查，`uid←staff.id`、`employee_id←staff.code`、`employee_name←staff.name`、`card_id/new_card_id←旧卡/新卡`、`dis_meal_type←staff.meal_type`。补录写 `meal_type=2/3、edible_time=补录日零点、dtime=补录日、bc_info=消费成功、bl=1、del=1`，整批共用 `blsystemcode`，`addtime` 为草稿打开时间，`bltime` 为保存时间，`bluser` 为当前账号 `truename`，`bl_info` 为备注，`ip` 由服务端请求取得。待审核行不计入正式刷卡。 |
| 一键漏卡补录 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `UB_ERP_Dining_meal_log` + `UB_ERP_Hr_staff` | 以一名有效员工和一个月为范围，先把 `Dining_meal` 的 `del=0、pass=1、dis_lx=2/3` 合并为员工+日期+餐别，再按饭堂配置 `two2/three2` 过滤已过结束时间的餐次；`meal_log.del=0` 正式流水及 `bl=1、del=1` 待审核补录均排除。提交时重新核对并按 `DiningSwipe:员工ID:日期:餐别` 加锁，写入字段沿用打卡消费补录，固定 `bc_info=消费成功、bl=1、del=1`，同次共用 `blsystemcode`，`bl_info` 自动写员工姓名和月份。不新增表、字段或索引。 |
| 补录管理与审核 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal_log` | 全历史列表按 `bl=1` 读取，以 `blsystemcode+dtime+meal_type+bluser+addtime` 组成逻辑批次，解决历史 `blsystemcode` 碰撞；`ROW_NUMBER()` 分页并以 `COUNT(1) OVER()` 同次返回总数。全批 `del=1` 为待审核、全批 `del=0` 为已审核，混合状态禁止操作。审核按 `uid+dtime+meal_type` 取得 `DiningSwipe` 锁，发现本批次外 `del=0` 流水即整批回滚，否则整批改 `del=0`；反审整批改回 `del=1`。展开明细卡号按 `new_card_id→card_id` 回退。不新增表或索引。 |
| 打卡消费记录搜索 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal_log` | `GET /api/canteen/records/consumptions` 只读正式流水 `del=0` 且餐别 `2/3`；默认当月及前两个月，可按 `dtime` 范围、`employee_id/employee_name`、`meal_type`、`card_id/new_card_id` 模糊筛选。`bl=1` 标为补录（操作人员取 `bluser`），否则为刷卡。卡号优先 `new_card_id`；备注优先 `bl_info` 回退 `remark`。`ROW_NUMBER()` 分页，不读报餐表、不含待审补录。 |
| 个人用餐记录 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal` + `_meal_log` + `UB_ERP_Dining` | `GET /api/dining/profile/meals` 只读取报餐会话本人；默认当天前23天至未来7天，`scope=all` 可查全部历史，固定10天分页。午餐、晚餐按日期合并为一行；报餐按 `uid+dis_dtime+dis_lx` 合并，成功流水按 `uid+dtime+meal_type` 合并；`bc_info=0/消费成功` 都视为已打卡，已报餐但未刷卡时按配置午餐 `two2`、晚餐 `three2` 判定未打卡或漏卡。 |
| 刷卡终端授权 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_machine` | `GET/POST /api/dining-terminal/*` 使用服务端解析的请求IP精确匹配 `ip`；返回 `px/name/tdname` 展示终端。当前全部授权窗口通用，不使用 `jw` 做部门限制。 |
| 实际用餐流水 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_meal_log` | `POST /api/dining-terminal/swipe` 按 `uid+dtime+meal_type+del=0` 防重复，使用 `sp_getapplock` 锁定单个员工餐次后 INSERT。正常报餐写 `bc_info=0`，未报餐补餐写 `bc_info=消费成功`；`employee_id←staff.code`、卡号写 `card_id/new_card_id`、时间写 `edible_time`、终端写 `ip`。旧统计继续读取该表。 |
| 报餐月份准备 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_report_month` | `month_key` 为 `YYYYMM`，一行代表该月已由管理员准备；`enabled=1` 才生效。不存在该月记录时员工端一律不可报。删除月份记录时一并删除该月特殊日期，仅超级管理员可执行。 |
| 报餐特殊日期 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_report_block` | 历史“禁报日期”表升级复用。`month_key` 对应已准备月份；`start_date/end_date` 只允许同月单日或连续区间；`report_status=allowed/blocked` 覆盖工作日可报、周末不可报的默认规则；同月启用规则禁止重叠。旧人工禁报记录迁为 `blocked`。 |
| 禁报日期开放例外 | `${DINING_DB_DATABASE}.dbo.UB_ERP_Dining_report_exception` | 常设或临时规则按部门 `in_bm_systemcode`、员工 `id` 匹配；只在月份已准备且默认/特殊规则为不可报时开放。法定节假日、调休不参与任何报餐日期判断。 |
...
## UB_ERP_Hr_room_use（宿舍电费）

- 业务：电费管理中心单房核算、房间列表「删除电费」、**一键录入（电费）**批量导入。
- 接口：`POST /api/hr/dormitory/electric/settle`；`POST /api/dorm/delete-electric`；`POST /api/hr/dormitory/electric/batch-preview`（预览不写库）；`POST /api/hr/dormitory/electric/batch-import`（确认导入，同月覆盖先删后插）。
- 关键字段：`room_code`（对 `UB_ERP_Hr_room.s_code`）、`tj_date`（如 `2026-7`）、`c_star`（上期）、`c_this`（本期）、`c_electric`、`c_money`、`c_yh_electric`、`c_sum_money`、`del`、`pass`。
- 一键录入校验：Excel A 房号 / B 上期 / C 本期；上期须与系统期望一致；本期不得小于上期；无人在住或房号不存在则跳过。