...
`keyword` 同时模糊匹配 **`name`**、**`code`**、**`card_number`**。可选 `in_bm` 精确匹配部门名称（与关键词可同时用；空则不限部门）。旧数据常见只有 `in_bm`、GUID 空或错，故列表不按 `in_bm_systemcode` 筛。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/hr/staff` | 分页：`page`、`pageSize`；`pass`；`del`（`0` 在职 / `1` 离职）；`keyword`；可选 `in_bm`（部门名称精确匹配，空=全部） |
...
- 默认分页：20
- 筛选：部门下拉（搜索框左侧，默认全部；点「查询」生效）/ 显示未审核 / 显示离职员工（无回收站）
- 列表**无**「批量更新」入口（避免工具栏单独占一行）
