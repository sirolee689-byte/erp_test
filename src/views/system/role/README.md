...
- **查**：`New_UB_ERP_System_role` 分页列表，`GET /api/roles`（支持 `status` 双视图、`keyword` 模糊搜角色名与描述；含 `Permissions` 列）。
...
- 表：`New_UB_ERP_System_role`（字段见上级目录 `rbac_design.md`）。
...
- 删除前必须先禁用；若 `New_UB_ERP_User` 中仍有用户引用该角色，数据库外键或接口校验会阻止删除。
...
- 超级管理员不是角色权限，不能在本页分配；由操作员资料的 `New_UB_ERP_User.is_admin=1` 单独维护。
- 新增、修改、禁用、恢复、分配权限和删除成功后，由中央白名单写一条 `UB_Date_ERP_Operation_log`；失败请求不记录。
