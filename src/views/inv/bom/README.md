...
- **列表一键运算 CUT 下层倍率**：主列表「一键运算」调用 `POST /api/bom/usage-calc-legacy`，但按钮不再显示“旧”字样，也不再限制超级管理员。写入 `UB_ERP_Bom_cost` 时，CUT 行自身仍按隐藏前缀不写入成本表，但 **CUT- 中间层的 `kcac04` 会参与所有下层的逐层相乘**。示例：`CUT-BAGPQ3188A3/GRN<1-2>` 数量为 2，子编码 `LA-0240/GR3` 用量 0.1673，列表一键运算落库为 `0.3346`。详情页运算、批量运算和销售订单运算仍保持普通口径：CUT 系数按 1 跳过。该列表口径不复刻旧 ASP 固定层级的历史例外。
- **数据库变更**：本次无数据库变更。
