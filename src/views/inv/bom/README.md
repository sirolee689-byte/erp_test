...
- **回收站**：仅 `del=1`；操作「恢复」「彻底删除」；彻底删除仅 `UB_ERP_User.is_admin=1` 的超级管理员可见并可执行；与「显示未审核」互斥
