...
- **顶栏/列表无框**（2026-07-23）：「管理BOM资料 / BOM资料添加」顶栏与列表卡片对齐出入库（不加内容区外框线）。
...
- **基础资料 / 配件明细**：既有功能。
- **BOM用量表运算**：「运算」/「刷新」均请求 **`GET /api/bom/tree?systemcode=`**；其中「运算」会带 `preferTree=1` 强制只读树预览（不写 `UB_ERP_Bom_cost`），用于刷新树形展示。返回 **嵌套 `children` 的树**；单层 SQL **只按 `kcac01` 匹配**（不按 `del` 过滤，与配件 GET 一致，避免旧库 `del` 空值导致子层 0 行）；**`kcac01`/`kcac02` 用 500 长 nvarchar 比较**防截断。前端用量表对齐 **PI 追溯**：原生 `<table>` + **▶/▼ 三角** + 展开 `Set` 压可见行（不用 `el-table` 树，避免展开卡顿）；工具条「展开全部 / 关闭全部」。该表已移除「备用损耗(kcaa33)」列，改为「合计(kcac06)」列，公式为 **`用量(kcac04) × (1 + 损耗(kcac05))`**；其中「用量/合计」统一按 **6 位小数去尾 0** 展示（仅改显示，不改后端计算与落库精度）。
- **BOM用量表单独展开（2026-07）**：默认**不**全展开，只显示顶层配件；点**三角（▶/▼）**或**编码**可单独展开/收起该行。展开时**递归打开该支下全部层级**（一次改 `Set`，与 PI 同交互样式、近零延迟）；再点同一行则收起该支；可同时展开多支。工具条保留「展开全部 / 关闭全部」，旁有提示文案。
...
- **编码列固定宽（2026-07）**：查看详情 / 编辑 / 联动查看大弹窗配件明细「编码」列改为固定 **`width="280"`**（按样例 `CUT-BAGPQ2803G1/MO<1-10>` 完整显示），不再按内容自适应。DIY：三处 `label="编码"` 的 `width` 数字。
- **查看详情配件明细 UI（2026-07）**：普通「查看详情」配件 Tab **无勾选列**、**不显示「添加配件 / 刷新」**（只读）；`parts-edit` 独立页仍保留添加工具条。编辑弹窗操作列「变更配件」与「编辑配件」共用 `.bom-part-view-action-btn` / `.bom-part-edit-child-action-btn`，字号与「管理BOM资料」一致。挂视口底横条后：全局 CSS **只藏横条、保留纵滚**；配件铺满区另让 `.erp-table-viewport-hscroll-host` 吃满外框高度（否则 `height:100%` 失效、表被裁切却滚不动）。DIY：`index.vue` 搜 `bom-parts-table-outer--fill`；`element-override.scss` 搜 `erp-table-viewport-hscroll-active`。
...
- **BOM 分类筛选**：工具栏下拉来自 **`UB_ERP_Bom_code`**（按 `id` 排序，展示 `flag1` 如「产品」）；查询传 **`bom_code_id`**，按该分类 **`flag5` 前缀** 匹配物料编码 `kcaa01`；默认「全部分类」。列表「分类」列仍为材料分类（`New_UB_ERP_Stocks_material` / `kcaa05`，旧表名 `Bom_material`），与筛选项不是同一张表。**仅改分类或裁片过滤下拉不会刷新列表**，须点「查询」或关键词回车。
...
- **回收站**：仅 `del=1`；操作「恢复」「彻底删除」；彻底删除仅 `New_UB_ERP_User.is_admin=1` 的超级管理员可见并可执行；与「显示未审核」互斥
...
- **PQ 成本排序补值**：只有主 BOM 编码以 `PQ-` 开头的一键运算会写入 `UB_ERP_Bom_cost.px`；规则为明细 `kcaa01` 找 `UB_ERP_Bom_000.kcaa05`，再用该分类编号匹配 `New_UB_ERP_Stocks_material.code` 取 `px`。读取成本 BOM 用量表缓存时，`PQ-` 主 BOM 按 `px` 从小到大显示，`px` 为空的排在后面；`BAG-`、`TAG-` 等纸格导入下级资料的一键运算不写 `px`，也不按 `px` 改排序，避免改变纸格导入后的排序。
- **列表/详情/批量运算 CUT 下层倍率**：主列表「一键运算」与主页「批量运算（当前页）」继续走写库口径（分别调用 `POST /api/bom/usage-calc-legacy`、`POST /api/bom/usage-calc-batch`，批量内部 `legacyCutMultiplier=true`）并写入 `UB_ERP_Bom_cost`。详情「BOM用量表运算」里的「运算」改为只读树预览，不再写库。写入 `UB_ERP_Bom_cost` 时，CUT 行自身仍按隐藏前缀不写入成本表，但 **CUT- 中间层的 `kcac04` 会参与所有下层的逐层相乘**。示例：`CUT-BAGPQ3188A3/GRN<1-2>` 数量为 2，子编码 `LA-0240/GR3` 用量 0.1673，落库为 `0.3346`。无缓存时 `GET /api/bom/tree` 的 `flatCostUsageRaw` 预览也用同一 legacy 平铺，避免点运算前后数字不一致。`POST /api/bom/usage-calc`（普通口径：CUT 系数按 1 跳过）仍保留供兼容/集成测，BOM 主界面写库入口不再依赖它。该口径不复刻旧 ASP 固定层级的历史例外。
- **数据库变更**：本次无数据库变更。
- **已知问题/下一步**：页码 `当前页/总页数` 在 Chrome/Edge 打印预览较稳定；Firefox 等浏览器可能无总页数或版式略有差异。若列过多被裁切，可在系统打印对话框中临时改横向。打印默认：表体 **14px / 字重 700**，抬头 **17px**（超长编码自动缩小，下限 9px）；仅明细列数据覆盖为 `Arial Unicode MS`、11px、正常字重。列宽比例编码 15%、名称/规格各 20%、单位 5%、备注 10%、用量/损耗各 11%、合计 8%（均可通过上述变量调整）。极端超长编码缩到 9px 下限后仍可能触达页宽，属可接受边界。
...
- **点行展开（2026-07）**：左边展开箭头列已全局隐藏；点主表行即可展开/收起明细（操作钮、展开区内点击不触发）；「展开全部」仍可用。
...
- 主列表会关联 `New_UB_ERP_Stocks_material` 显示分类名、`UB_ERP_Stocks_colorcode` 显示颜色名、`UB_ERP_Finance_currency` 显示报价/采购币别名；采购、外协、自产、是否保税、客户供应等布尔字段按 `1=是` 展示。
...
- **配件明细列顺序**：查看详情为：序号、操作、编码、用量、损耗、合计、搭配、名称、规格、单位、分类、客户款号、工厂款号、组别、生产车间编码、颜色（**无勾选列**）；编辑配件另含自定义「选择」列。界面不再显示说明/备注、单价、成本合计，但不改这些字段的保存值。`GET /api/inventory/bom/parts/:systemcode` 对分类/款号/组别/车间编码与现有编码/名称/规格/颜色一样，优先取子 BOM 主档当前值；主档不存在或该字段为空时才回退配件行快照。生产车间只显示 `kcaa15` 编码。用量、损耗、合计继续按现有规则去除无意义尾 0。仅编辑配件表的行操作为「变更配件 / 编辑配件」；编辑配件保留原下钻方式、不显示下划线，搭配输入框保留最多 100 字限制但不显示 `0/100`。编辑中的用量最多 6 位、损耗最多 2 位，均去除无意义尾 0；这里不再使用 `el-input-number` 的 `precision`（该属性会强制补零），而是在录入结束后规整数值，因此不改变保存精度、损耗百分比换算或合计计算。
