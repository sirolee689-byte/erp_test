...
- **一键运算(旧)**：仅在「显示未审核」的主列表操作列中显示，位置紧挨「审核」右侧，且仅超级管理员（`UB_ERP_User.is_admin=1`）可见、可用；后端会实时查库再次校验，前端隐藏不是权限依据。该按钮调用 `POST /api/bom/usage-calc-legacy`，只改变一条口径：`CUT-` 中间层的 `kcac04` 也会参与所有下层的逐层相乘；CUT 行本身仍按隐藏前缀不写入成本表。普通「一键运算」、批量运算、销售订单运算仍保持 CUT 系数按 1 跳过；旧按钮不复刻旧 ASP 固定层级的历史例外。
- **数据库变更**：本次无数据库变更。
