...
- 操作列：查看详情（弹窗）、复制（剪贴板 JSON）、编辑（占位提示，待后续对接保存接口）。

## 数据库说明

- 目标表默认 `dbo.bom_000`，可通过环境变量 `INV_BOM_MASTER_TABLE` 改为其他表名（仅字母数字下划线）。
- 接口假设存在字段：`kcaa01`～`kcaa14`、`sign`、`version`、`pass`、`del`（与项目其它模块一致的审核与逻辑删除约定）。若旧库缺列，请在 Navicat 补齐或联系开发调整 SQL。
...
- 「编辑」尚未对接 `PUT` 保存与校验，仅保留入口与未审核可点状态。
