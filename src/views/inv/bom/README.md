...
- **`GET /api/inventory/bom/parts/:systemcode`**：`kcaa01`/`kcaa02`/`kcaa03`/`kcaa11` 优先按 **`UB_ERP_Bom_000.kcaa01`**（在册主档，`OUTER APPLY` **TOP 1**）展示；无匹配则用配件表原列。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/inv/bom/list` | 分页列表；`recycled=1` 回收站；否则 `pass` + 在册 `del`；可选 **`bom_code_id`**（`UB_ERP_Bom_code.id`，按 flag5 前缀筛 `kcaa01`）；返回含 **`systemcode`** |
