...
- 顶栏：对齐供应商资料——「管理结算方式」/「结算方式添加」（已去掉工具条「新增结算方式」）
...
- **添加 / 编辑**：均为**当前页表单**（不再用弹窗）；保存或「返回列表」后回到管理列表
- 表单字段：编码（A=250）/ 名称（A）/ 天数（B=80，旁注「天」）/ 备注（C=500）；编辑时编码只读
- 审核 / 反审：二次确认弹窗；审核后进入默认列表（已审核）
- 删除（软删）：二次确认弹窗；仅未审核且在册允许删除，删除后进入回收站
- 回收站：支持恢复与彻底删除（物理删除，不可恢复）；彻底删除仅 `New_UB_ERP_User.is_admin=1` 的超级管理员可执行
- 列表：操作列左固定；状态列显示已审/未审
