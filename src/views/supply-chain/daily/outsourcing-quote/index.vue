<template>
  <div class="erp-module-page pq-quote-page">
        ...
        <template #default>
          <el-table
           ...
           @row-contextmenu="onErpListRowContextMenu">
                ...
                <div v-loading="row.__linesLoading" class="expand-inner">
                  ...
                  >
                    ...
                    <el-table-column label="单价" width="100" show-overflow-tooltip>
                      <template #default="{ row: line }">{{ formatMoney(lineField(line, 'wxab04')) }}</template>
                    </el-table-column>
                    <el-table-column label="单价(含税)" width="112" show-overflow-tooltip>
                      <template #default="{ row: line }">{{ formatMoney(lineField(line, 'wxab05')) }}</template>
                    </el-table-column>
            ...
            <el-table-column label="外协报价日期" width="118" show-overflow-tooltip>
    ...
    <!-- 查看 -->
    <el-dialog v-model="viewVisible" title="查看外协报价" width="920px" destroy-on-close>
    ...
    <section v-show="editVisible" v-loading="editLoading" class="pq-edit-panel">
      ...
      <div class="edit-wrap">
        <el-tabs v-model="editActiveTab" class="pq-edit-tabs" @tab-change="onEditTabChange">
                ...
                <el-col :xs="24" :sm="14">
                  <el-form-item label="报价单号" required>
                    <div class="pq-docno-row">
                      <el-input v-model="basicForm.wxaa01" clearable placeholder="外协报价单号 wxaa01" />
                ...
                <el-col :xs="24" :sm="10">
                  <el-form-item label="报价日期">
                    <el-date-picker
                      ...
                      placeholder="选择报价日期（写入 addtime / wxaa02）"
                      style="width: 100%"
                    />
                  </el-form-item>
                </el-col>
          ...
          <el-tab-pane label="外协报价明细" name="lines">
...
watch(
  ...
  () => {
    for (const r of lineRows.value || []) {
      const rawEx = lineField(r, 'wxab04')
...
function displayQuotationNo(row) {
  const s = String(row?.wxaa01 ?? '').trim()
...
function quoteDateDisplay(row) {
  ...
  return formatDateCell(row?.wxaa02)
}
...
function validUntilDisplay(row) {
  ...
  return formatDateCell(row?.wxaa07)
}
...
function quoteSummaryRow(row) {
  ...
  const tax =
    row.pq_tax_amount != null && row.pq_tax_amount !== ''
      ...
      : inc - ex
  // 列表汇总：多行时后端对 wxab04/wxab05 做 SUM，与「单 line 金额×行数」等价于按行累加
  return `总项数：${cnt}；含税总价：${formatMoney(inc)}；不含税总价：${formatMoney(ex)}；税点总价：${formatMoney(tax)}`
}
...
function applyInclToEx(row) {
  if (!row) return
  const places = Math.min(8, Math.max(0, Number(basicForm.decimalPlaces) || 4))
  const rawIncl = lineField(row, 'wxab05')
  ...
  row.wxab04 = roundByDecimals(incl / denom, places)
}
...
function lineHasNumericPrice(r) {
  const ex = lineField(r, 'wxab04')
  const incl = lineField(r, 'wxab05')
  const exOk =
...
function syncLinePricesForSubmit(r) {
  ...
  const rawEx = lineField(r, 'wxab04')
  const rawIncl = lineField(r, 'wxab05')
  ...
  const denom = 1 + taxPct / 100
  if (hasEx) {
    r.wxab04 = roundByDecimals(exN, places)
    r.wxab05 = roundByDecimals(exN * denom, places)
  } else if (hasIncl && denom > 0) {
    r.wxab05 = roundByDecimals(inclN, places)
    r.wxab04 = roundByDecimals(inclN / denom, places)
  }
}
...
function sanitizeLinesForApi() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const taxPct = normalizeTaxInput(lineField(r, 'Tax'))
    const ex = roundByDecimals(lineField(r, 'wxab04'), places)
    const incl = roundByDecimals(Number(lineField(r, 'wxab05')), places)
    out.push({
      kcaa01: code,
...
const expandPrefetch = createExpandPrefetch({
  fetchBatch: async (ids) => {
    const { data } = await axios.get('/api/supply-chain/outsourcing-quotations/lines/batch', { params: { ids: ids.join(',') } })
    if (data.code !== 200) throw new Error(data.msg)
    return data.data || {}
  },
  fetchSingle: async (id) => {
    const res = await axios.get(`/api/supply-chain/outsourcing-quotations/${id}/lines`)
...
async function loadData() {
  ...
  try {
    ...
    const res = await axios.get('/api/supply-chain/outsourcing-quotations/list', { params })
...
/** 根据主表已存 wxaa05 / rmb 反推下拉 value */
function resolveCurrencyCodeFromHeader(header) {
  const c05 = String(lineField(header, 'wxaa05') ?? '').trim()
...
async function fetchSuggestedDocNo() {
  try {
    const res = await axios.get('/api/supply-chain/outsourcing-quotations/suggest-doc-no')
    const s = res?.data?.data?.suggested
    if (s) basicForm.wxaa01 = String(s)
  } catch {
    /* 网络或权限失败时不阻断弹窗 */
  }
}
...
async function checkQuotationDocNo() {
  const code = String(basicForm.wxaa01 ?? '').trim()
  if (!code) {
  ...
  try {
    const res = await axios.get('/api/supply-chain/outsourcing-quotations/check-doc-no', {
      params: { wxaa01: code },
    })
...
async function searchSuppliers(q) {
  ...
  try {
    const res = await axios.get('/api/supply-chain/outsourcing-quotations/supplier-options', {
...
function buildHeaderForSubmit() {
  ...
  /** @type {Record<string, unknown>} */
  const header = {
    wxaa01: String(basicForm.wxaa01 ?? '').trim(),
...
async function openBomDetail(line) {
  ...
  try {
    const res = await axios.get('/api/supply-chain/outsourcing-quotations/bom-detail', {
      params: { kcaa01: code },
    })
...
async function openEdit(row) {
  ...
  try {
    ...
    basicForm.wxaa01 = String(lineField(header, 'wxaa01') ?? '').trim()
    ...
    const c2 = lineField(header, 'wxaa02')
    basicForm.quoteDate = normalizeHeaderDateForPicker(addt || c2) || formatTodayYmd()
    basicForm.validUntil = normalizeHeaderDateForPicker(lineField(header, 'wxaa07'))
    ...
    lineRows.value = lines.map((raw) => {
      ...
      const rawEx = lineField(x, 'wxab04')
      const ex = Number(rawEx)
      const hasEx =
        rawEx !== '' && rawEx != null && rawEx !== undefined && Number.isFinite(ex)
...
async function submitEdit() {
  ...
  if (!String(basicForm.wxaa01 ?? '').trim()) {
  ...
  try {
    ...
    if (editMode.value === 'create') {
      await axios.post('/api/supply-chain/outsourcing-quotations', { header, lines })
      ElMessage.success('新增成功')
    } else {
      await axios.put('/api/supply-chain/outsourcing-quotations', {
        id: editId.value,
        header,
        lines,
      })
...
async function auditRow(row) {
  ...
  try {
    await axios.put('/api/supply-chain/outsourcing-quotations/audit', { id: row.id })
...
async function unauditRow(row) {
  ...
  try {
    await axios.put('/api/supply-chain/outsourcing-quotations/unaudit', { id: row.id })
...
async function softDeleteRow(row) {
  ...
  try {
    await axios.delete(`/api/supply-chain/outsourcing-quotations/${row.id}`)
...
async function restoreRow(row) {
  ...
  try {
    await axios.put('/api/supply-chain/outsourcing-quotations/restore', { id: row.id })
...
async function permanentDeleteRow(row) {
  ...
  try {
    await axios.delete(`/api/supply-chain/outsourcing-quotations/${row.id}/permanent`)
