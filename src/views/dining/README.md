...
- 报餐管理页按日期展示明天起未来 30 天，每天可逐项报午餐、晚餐；另提供“一键报午餐、一键报晚餐、一键全报、一键取消全部”。批量操作只处理仍可操作的餐次，禁报日、未准备月份、截止日期和已是目标状态的餐次自动跳过；桌面双列、手机单列，不提供菜式选择。
...
- `PUT /api/dining/meals/batch`：按 `action=lunch|dinner|all|cancel` 统一处理当前未来 30 天；服务端重算日期资格后，在一个事务中集中读取已有报餐和统一餐兼容项，再批量写入或软删除旧饭堂报餐记录，避免逐日期、逐餐别反复往返数据库。
