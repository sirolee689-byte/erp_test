...
- 列表：`GET /api/inventory/units/list`，物理表 **`UB_ERP_Stocks_unit`**；`ROW_NUMBER()` 分页（SQL Server 2008 R2）；默认 **每页 20**、按 **`id` 降序**。
...
- **业务表审计列**（建议 `NVARCHAR(50)` 存时间串；`uid` 为 `INT`；见 `CONTEXT.md` 第三节）：
  - **`uid` / `uname` / `utruename`**：录入人（对应 `New_UB_ERP_User.UserID`、`UserCode`、`UserName`）；
  - **`addtime`**：新增时写入；
...
```sql
ALTER TABLE dbo.[UB_ERP_Stocks_unit] ADD uid INT NULL;
ALTER TABLE dbo.[UB_ERP_Stocks_unit] ADD uname NVARCHAR(50) NULL;
ALTER TABLE dbo.[UB_ERP_Stocks_unit] ADD utruename NVARCHAR(50) NULL;
ALTER TABLE dbo.[UB_ERP_Stocks_unit] ADD addtime NVARCHAR(50) NULL;
ALTER TABLE dbo.[UB_ERP_Stocks_unit] ADD edittime NVARCHAR(50) NULL;
ALTER TABLE dbo.[UB_ERP_Stocks_unit] ADD deltime NVARCHAR(50) NULL;
```
...
新增、审核、反审、删除、恢复、彻底删除登记为中央白名单，由 `operationAuditMiddleware` 写入 **`UB_Date_ERP_Operation_log`**，失败请求不记录。

## 已知问题 / 下一步

- 未审行内编辑、回收站彻底删除：可按业务需要再迭代；彻底删除仅 `New_UB_ERP_User.is_admin=1` 的超级管理员可执行（当前与最小标准件一致）。
- **UI（2026-07）**：主列表操作列已按 BOM 资料标准统一（`ErpTableActions` + `plain` 语义色；未审视图显示审核/删除，已审视图显示反审）。
- **分页（2026-07）**：每页条数下拉已全站统一为 5/10/20/50/100/200/500/1000（`ERP_PAGE_SIZE_OPTIONS`）；本页默认仍为 **20 条/页**，未改。
