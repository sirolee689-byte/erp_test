...
- 审核 / 反审：`PUT /api/inventory/unit-conversion/audit`、`PUT /api/inventory/unit-conversion/unaudit`，body **`{ id }`**；每次更新写 **`edittime`**（规则 16）。
...
- 新增：`POST /api/inventory/unit-conversion`，body **`{ unit_name, unit_name_tow, change_bl }`**；**`uid`/`uname`/`utruename`** + **`addtime`** 由后端从登录态写入（规则 16）；`pass=0`、`del=0`；`id` 自增（`OUTPUT INSERTED.id`）。
...
- **规则 16 审计列**（建议 `NVARCHAR(50)` 存时间串；`uid` 为 `INT`）：`uid`、`uname`、`utruename`、`addtime`、`edittime`、`deltime`。
