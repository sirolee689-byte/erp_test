...
- 回收站彻底删除：`DELETE /api/inventory/color-code/:code/permanent`，**仅当 `del=1` 且当前用户 `New_UB_ERP_User.is_admin=1`** 时物理删除该行。
...
新增、修改、审核、反审、删除、恢复、彻底删除均登记为中央白名单，由 `operationAuditMiddleware` 写入 **`UB_Date_ERP_Operation_log`**，每次成功操作只写一条可读中文日志。
