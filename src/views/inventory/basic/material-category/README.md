...
- 列表：`GET /api/inventory/material-category/list`，物理表 **`Bom_material`**；`ROW_NUMBER()` 分页（SQL Server 2008 R2）；默认 **每页 20**、按 **`id` 降序**。
...
```sql
ALTER TABLE dbo.[Bom_material] ADD uid INT NULL;
ALTER TABLE dbo.[Bom_material] ADD uname NVARCHAR(50) NULL;
ALTER TABLE dbo.[Bom_material] ADD utruename NVARCHAR(50) NULL;
ALTER TABLE dbo.[Bom_material] ADD addtime NVARCHAR(50) NULL;
ALTER TABLE dbo.[Bom_material] ADD edittime NVARCHAR(50) NULL;
ALTER TABLE dbo.[Bom_material] ADD deltime NVARCHAR(50) NULL;
```
