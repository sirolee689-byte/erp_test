...
- 只读面板组件：`PiBomViewerPanel.vue`（独立窗与销售订单侧共用）；**PI_BOM树形**对齐 BOM用量表/PI追溯：原生 `<table>` + **▶/▼ 三角** + 展开 `Set` 压可见行（默认只显示顶层，点三角/编码时整支全开；不用 `el-table` 树与 `default-expand-all`，避免大树卡顿）
...
- 入口：首页顶部 `PI-BOM物料批量替换`；表单字段（每行一个）：PI号、PQ编码（留空则全部款）、物料源编码、目标物料编码、搭配（留空则仅匹配 `Describe` 为空的行；有搭配如「叻色」须填写后才能替换）、密钥（与 ERP 内核核心密钥 `ERP_CORE_CONFIG_KEY` 一致，正确才能立即执行）。
- 体验优化：PI/PQ/物料源/目标/搭配输入框均支持“输入触发下拉联想”；下拉仅显示编码，降低误输风险。
- 立即执行：先校验核心密钥，再预检命中行数并二次确认，再从 `UB_ERP_Bom_000`（旧系统称 UB_ERP_Bom_000）读取目标物料档案，批量更新 `UB_ERP_Bom_Sales_list` 中匹配行的物料属性（`kcaa01`~`kcaa35` 及同名快照列）。
...
- PI-BOM 主档保存、配件保存和实际物料替换由中央白名单写入 `UB_Date_ERP_Operation_log`。
- 物料替换 `dryRun=true` 仅做预检，不写操作日志；只有实际替换成功才记录；请求体中的核心密钥 `key` 会脱敏，不写入日志。
