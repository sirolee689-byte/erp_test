<template>
  <div class="erp-module-page stock-in-stats-page">
    ...
    <section class="report-shell">
      ...
      <h2 class="report-title">入库统计表</h2>
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" class="query-form">
        ...
        <div class="query-grid">
          ...
          <el-form-item label="入库类别">
            <el-select v-model="form.inboundType" clearable placeholder="全部类别">
      ...
      <div v-if="queryProgress.active" class="query-progress-panel">
        ...
        <p class="query-progress-text">正在统计入库明细，已等待 {{ queryProgress.elapsedSec }} 秒。</p>
...
defineOptions({ name: 'InventoryAnalysisStockInStats' })

const MENU_PATH = 'inventory/analysis/stock-in-stats'
const ALL_WAREHOUSE = '__ALL__'
const REPORT_TITLE = '入库统计表'
const COLUMN_SETTING_KEY = 'erp.stockInStats.columnSetting.v1'
...
const inboundTypeOptions = [
  { value: '0', label: '0 其他入库' },
  { value: '1', label: '1 采购入库' },
  { value: '2', label: '2 外协入库' },
  { value: '3', label: '3 外协退料' },
  { value: '4', label: '4 生产入库' },
  { value: '5', label: '5 生产退料' },
  { value: '6', label: '6 成品退货' },
  { value: '7', label: '7 盘盈入库' },
  { value: '8', label: '8 加工入库' },
  { value: '9', label: '9 其他入库' },
]
...
const baseColumns = [
  ...
  { key: 'inboundTypeLabel', label: '入库类别', width: 110 },
...
function currentWarehouseLabel() {
  ...
  return hit?.name || form.warehouseCode || ''
}

async function loadPrintConfig() {
  try {
    const { data } = await axios.get('/api/stock-in-stats/print-header')
    const cfg = data?.data ?? {}
...
async function fetchWarehouses(keyword = '') {
  try {
    const { data } = await axios.get('/api/stock-in-stats/warehouse-options', { params: { keyword } })
...
async function fetchMaterials(keyword = '') {
  try {
    const { data } = await axios.get('/api/stock-in-stats/material-options', { params: { keyword } })
...
async function queryMaterialCodeSuggestions(keyword, cb) {
  ...
  cb(list.map((item) => ({ value: String(item?.code ?? '').trim() })))
}

async function fetchCategories(keyword = '') {
  try {
    const { data } = await axios.get('/api/stock-in-stats/category-options', { params: { keyword } })
...
async function fetchRelatedParties(keyword = '') {
  try {
    const { data } = await axios.get('/api/stock-in-stats/related-party-options', { params: { keyword } })
...
async function loadReport() {
  ...
  try {
    ...
    const { data } = await axios.get('/api/stock-in-stats/report', { params, timeout: 180000 })
    ...
    ElMessage.error(String(e?.response?.data?.msg ?? e?.message ?? '读取入库统计表失败'))
  } finally {
    loading.value = false
    stopQueryProgressTimer()
  }
...
async function exportReportXlsx() {
  ...
  for (const row of displayRows.value) {
    ...
    styleExportRow(added, { bold: row.rowType === 'subtotal' || row.rowType === 'total' })
  }
...
.query-grid :deep(.el-autocomplete),
