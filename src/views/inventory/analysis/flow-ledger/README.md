...
- 弹窗不显示物料唯一码；物料编码提供远程联想但不强制选择，候选只显示物料编码。复制或手工填写正确编码后可直接查询，后端按明细 `kcaa01` 精确匹配。
- 选择联想项时自动带出物料名称、规格、单位；手工填写时不清空已输入编码，名称、规格、单位保持为空。
...
- 入库、出库均统计主表已审核和未审核 `pass in (0,1)`、主从未删除 `del=0` 的数据；未审核流水同样参与期初与逐行结存，注释前显示红色“(未审)”标记；明细表不按 `pass` 过滤。
- 入库日期用 `h.kcan02`，入库数量用 `l.kcao03`。
- 出库日期用 `h.kcap02`，出库数量用 `l.kcaq03`。
- 不使用旧系统真实中间表 `UB_ERP_Stocks_acc`，也不会清空或写入该表。
- 上期结存 = 查询开始日期之前已审和未审入库合计 - 已审和未审出库合计。
