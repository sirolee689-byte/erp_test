<template>
  <div class="stock-batch-window">
    <header class="stock-batch-header">
      <h1 class="stock-batch-title">采购入库批量添加明细</h1>
      <p class="stock-batch-subtitle">采购单号：{{ sourceOrderNo }}　供应商：{{ supplierCode }} {{ supplierName }}</p>
    </header>
    ...
    <el-skeleton :loading="loading" animated :rows="10">
      <template #default>
        <el-empty v-if="!rows.length && !loading" :description="errorMsg || '该采购单下暂无可选明细'" />
          ...
          <table class="stock-batch-table">
            <thead>
              <tr>
                ...
                <th>采购币别/汇率</th>
                <th>使用单位</th>
                <th class="col-num col-need">需入数量</th>
                <th class="col-num col-overflow">可超量入库数</th>
                <th>PO/PI</th>
                <th>备注</th>
            ...
            <tbody>
              <tr v-for="row in rows" :key="row.lineKey" :class="{ 'stock-batch-row--picked': isPicked(row) }">
                ...
                <td class="col-pending">{{ row.pendingReturnText || '—' }}</td>
                <td class="col-num">{{ formatNum(row.actualInboundQty) }}</td>
                <td class="col-num">{{ formatNum(row.returnQty) }}</td>
...
defineOptions({ name: 'inventory-daily-stock-in-purchase-batch-window' })
...
function togglePick(row) {
  ...
  // 深拷贝为纯对象，避免 Vue 响应式代理导致 postMessage 报 DataCloneError
...
async function loadRows() {
  ...
  try {
    ...
    const { data } = await axios.get('/api/stock-in/purchase-batch-lines', {
      params: {
...
function onSaveAccepted(lineCount) {
  ...
  setTimeout(() => {
    window.close()
    setTimeout(() => {
      closeHint.value = '明细已带回入库单。如浏览器未自动关闭，请直接关闭本窗口。'
    }, 300)
  }, 300)
}
  ...
  function onReply(event) {
    ...
    if (data.type === STOCK_BATCH_MSG_REJECTED) {
      ...
      if (data.reason === STOCK_BATCH_REJECT_SOURCE_MISMATCH) {
        ElMessage.warning('采购单号已变更，请重新打开批量添加')
      } else if (data.reason === STOCK_BATCH_REJECT_SUPPLIER_MISMATCH) {
        ElMessage.warning('供应商已变更，请重新打开批量添加')
      } else {
