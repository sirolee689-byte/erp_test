...
- **主列表操作列宽（2026-07）**：非 BOM 主列表统一用 `getErpTableActionsColWidthByRows`，按当前页每行实际可见按钮文案（含 `hasPageAction` 权限、回收站、审核状态）取最大宽度；权限不足不会预留隐藏按钮的空白。操作单元格左侧约 10px、右侧最多约 5px。**BOM 模块整模块仍除外**；子表、展开明细和弹窗内操作钮也不在此次范围。标杆：采购订单 / 销售订单 / 颜色编码。
...
## 表格展开箭头列（全局隐藏 · 2026-07）

- **现象**：订单/出入库等主列表已支持「点行展开明细」，左边 `type="expand"` 箭头列显得多余。
- **全站处理**：`element-override.scss` 搜索「**箭头列视觉隐藏**」——当前 Element Plus 实际类名为 `.table__expand-column`，将其收成 0 宽并隐藏 `.el-table__expand-icon`；各页面 `type="expand"` 同时显式设为 `width="1"`，避免组件默认预留 48px 空列。**模板仍保留** `type="expand"`（明细插槽依赖它）。
- **交互**：统一靠点行开合；BOM「转向物料查询」另保留工具条「展开全部 / 收起全部」。DIY：删掉该 CSS 段即可恢复箭头列。
